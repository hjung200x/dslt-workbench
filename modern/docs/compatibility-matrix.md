...
| Optional CUDA backend | `Auto`, `CPU`, `CUDA` execution policy | scaffolded | Hosted CUDA 13.2 NVCC/MSVC build plus RTX 4060 runtime parity completed through seeded watershed, including exact masks/labels, cancellation, validation, and mixed-operation memory fixtures; DSLT segmentation and automated GPU-fleet evidence pending |
