...
| Optional CUDA backend | `Auto`, `CPU`, `CUDA` execution policy | scaffolded | Hosted CUDA 13.2 NVCC/MSVC build plus RTX 4060 runtime parity completed through height/depth projection; 6/18/26 connected-components kernels, deterministic label parity, cancellation, and memory fixtures implemented with runtime evidence pending; H-minima, watershed, DSLT segmentation, and automated GPU-fleet evidence pending |
