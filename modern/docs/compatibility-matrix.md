...
| Optional CUDA backend | `Auto`, `CPU`, `CUDA` execution policy | scaffolded | Hosted CUDA 13.2 NVCC/MSVC build plus RTX 4060 runtime parity completed for copy, window/level, and global 2D/3D threshold, including cancellation, explicit unsupported-operation, VRAM preflight, and 100-run memory fixture; remaining processing groups and automated GPU-fleet evidence pending |
