...
| Optional CUDA backend | `Auto`, `CPU`, `CUDA` execution policy | scaffolded | Hosted CUDA 13.2 NVCC/MSVC build plus RTX 4060 runtime parity completed through the DSLT directional-threshold basis, including exact masks/labels, cancellation, validation, and mixed-operation memory fixtures; threshold sweep, iterative DSLT segmentation, and automated GPU-fleet evidence pending |
