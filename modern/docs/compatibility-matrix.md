...
| Optional CUDA backend | `Auto`, `CPU`, `CUDA` execution policy | scaffolded | Hosted CUDA 13.2 NVCC/MSVC build plus RTX 4060 runtime parity completed through connected components; H-minima fixed-point reconstruction, exact mask parity, cancellation, validation, and memory fixtures implemented with runtime evidence pending; watershed, DSLT segmentation, and automated GPU-fleet evidence pending |
