...
| LSM metadata | TIFF-compatible pixel decode plus Zeiss private tag 34412 | Not fully established | CZ_LSMINFO magic/size, X/Y/Z/C/T, meter-to-micrometer voxel calibration, C-fastest page order, planar samples, reduced-resolution IFD exclusion, channel names/RGBA colors, and timestamps are fixture-tested. Three public real LSM files match independent `tifffile 2026.5.15` CZYX pixels and metadata exactly; comparison with the unavailable original DSLT samples remains pending |
